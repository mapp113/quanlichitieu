...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerButton.setOnClickListener(v -> {
            ...
            if (PasswordUtils.checkUserInfoValid(password, username, fullName)) {
                // Chạy DB query trong luồng nền
                new Thread(() -> {
                    String salt = PasswordUtils.generateSalt();
                    String hashedPassword = PasswordUtils.hashPassword(password, salt);
                    appDatabase.getDatabase(this).userDao().insertUser(new User(username, hashedPassword,fullName,"",System.currentTimeMillis() ,salt));
                    runOnUiThread(() -> {
